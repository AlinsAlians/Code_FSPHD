...
def preprocess(rep, r_split):
    ...
    Vars_conj = ['bm', 'meteo', 'pop', 'ndvi'] # vars explicatives conjonctural
    ...
    for i in range(2): # année t-1 et t
        for j in range(5, 12): # mois de mai (5) à novembre (11)
            ...
            if 'pluie' in Vars_lstm:
                data_aggr['pluie{0}t-{1}'.format(j,1-i)] = data_rep.filter(regex='pluie_.*{0}\(.*t-{1}'.format(j,1-i)).sum(axis=1)
            if 'mais' in Vars_lstm:
                data_aggr['mais{0}t-{1}'.format(j,1-i)] = data_rep.filter(regex='{0}mais-.*\(.*t-{1}'.format(j,1-i))
